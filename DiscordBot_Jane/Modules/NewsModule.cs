...
namespace DiscordBot_Jane.Core.Modules
{
    ...
    public class NewsModule : ModuleBase<SocketCommandContext>
    {
        ...
        public NewsModule(IConfigurationRoot config)
        {
            _config = config;
        }

        [Command("introducera dig själv"), Alias("introducera dig", "introducera")]
        [Summary("Jane introducerar sig själv och beskriver vad hon kan göra, bland annat.")]
        ...
        private async Task SendIntroduction()
        {
            var builder = new EmbedBuilder()
                ...
                .AddField("Vad jag kommer att kunna göra", "- Påminna om mattevideos\n- Påminna om idrottslektioner\n- Svara på frågor angående när uppgifter ska lämnas in\n- Kompilera allt som ska lämnas in till ett enda dokument/kalender för att ge användare en överblick över t.ex. den kommande veckan.\n- Påminna om uppkommande prov inom vissa ämnen (t.ex. Matte eller Samhällskunskap)");
            var embed = builder.Build();

            await ReplyAsync("", false, embed);
        }

        [Command("changelog"), Alias("ändringslogg", "förändringar")]
        [Summary("Jane berättar vilka nya funktioner och fixar som har implementerats i den senaste updateringen.")]
        [RequireOwner]
        public async Task SendChangelogAsync()
        {
            await ChangelogAsync();
        }

        [Command("changelog"), Alias("ändringslogg", "förändringar")]
        [Summary("Jane berättar vilka nya funktioner och fixar som har implementerats i den senaste updateringen.")]
        [RequireOwner]
        public async Task SendChangelogAsync([Remainder]string remainder)
        {
            await ChangelogAsync();
        }
        ...
        public async Task ChangelogAsync()
        {
            ...
            var builder = new EmbedBuilder()
                .WithTitle("Källkod")
                .WithDescription($"Här är de nya förändringarna för version **{_config["changelog_version"]}**\n\n")
                .WithUrl("https://github.com/martinjonsson01/LBS_Jane")
                .WithColor(new Color(0xDE0190))
                .WithTimestamp(new DateTimeOffset(new DateTime(2018, 2, 26, 22, 0, 0)))
                .WithFooter(footer =>
                {
                    footer
                        .WithText("Version släppt")
                        .WithIconUrl(
                            "https://cdn.discordapp.com/avatars/153217095625211905/b20316e13a9d43e0ea1294208b9db6ef.png?size=32");
                })
                .WithThumbnailUrl(
                    "https://cdn.discordapp.com/avatars/409698620594454528/e6fcb735d4163612c92f5c66d7d2f66a.png?size=128")
                .WithAuthor(author =>
                {
                    author
                        .WithName("Jane")
                        .WithIconUrl(
                            "https://cdn.discordapp.com/avatars/409698620594454528/e6fcb735d4163612c92f5c66d7d2f66a.png?size=32");
                });

            if (ConfigUtils.GetListFrom("changelog_bulletpoints", _config) is List<string> bulletpoints)
            {
                foreach (var bulletpoint in bulletpoints)
                {
                    builder.AddField((bulletpoints.IndexOf(bulletpoint)+1).ToString(), bulletpoint);
                }
            }
